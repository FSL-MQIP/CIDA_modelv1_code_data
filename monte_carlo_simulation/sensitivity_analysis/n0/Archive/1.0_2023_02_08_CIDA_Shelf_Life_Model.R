...
initial_count <- read.csv("model_inputs/initial_distribution_2022_08_24.csv", header = TRUE)
...
initial_count_dist <- function(n){
  a <- runif(n, min = initial_count$min, max = initial_count$max)
  ans <- a*1.0
  return(ans)
}
...
h17 <- hist(day17_sim_output$total_count, 
            breaks = 20,
            xlab = "Bacterial concentration, log10(CFU/g)", 
            ylab = "Density", main = "Model output on day 17 of shelf life", 
            prob = TRUE, col = "white")
...
h22 <- hist(day22_sim_output$total_count, 
            breaks = 15,
            xlab = "Bacterial concentration, log10(CFU/g)", 
            ylab = "Density", main = "Model output on day 22 of shelf life", 
            prob = TRUE, col = "white")
...
date <- Sys.Date()
date <- gsub("-", "_", date)
file_name <- paste0("sensitivity_analysis/n0/output/sa_n0_", date,".csv")
