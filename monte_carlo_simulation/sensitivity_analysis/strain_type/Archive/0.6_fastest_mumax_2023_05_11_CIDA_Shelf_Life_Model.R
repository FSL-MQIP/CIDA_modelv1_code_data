...
lag_input <- read.csv("model_inputs/averaged_lag_2023_05_02.csv", header = TRUE)
...
st_frequency <- st_frequency %>%
  dplyr::select(cida_isolates, frequency)

s12_141 <- st_frequency$frequency[st_frequency$cida_isolates == "s12-0141"]
mod_s12_141 <- s12_141*0.6
mod_sum <- sum(st_frequency$frequency[st_frequency$cida_isolates != "s12-0141"])
st_frequency$mod_freq <- st_frequency$frequency + (st_frequency$frequency/mod_sum)*(s12_141 - mod_s12_141)
st_frequency$mod_freq[st_frequency$cida_isolates == "s12-0141"] <- mod_s12_141
...
lag_input$isolate <- tolower(lag_input$isolate)
...
for(i in 1:nrow(packages_sim_exp)){
  strain_index <- which(packages_sim_exp$isolate[i] == lag_input$isolate)
  strain_lag <- lag_input$ave_lag[strain_index]
...
sa <- shelf_life_sim %>%
  ...
  mutate(adj = 0.6) %>%
  mutate(param = "fastest_strain_s12_0141")

## --------------------------------Export---------------------------------------

date <- Sys.Date()
date <- gsub("-", "_", date)
file_name <- paste0("sensitivity_analysis/strain_type/output/sa_strain_type_", date,".csv")
