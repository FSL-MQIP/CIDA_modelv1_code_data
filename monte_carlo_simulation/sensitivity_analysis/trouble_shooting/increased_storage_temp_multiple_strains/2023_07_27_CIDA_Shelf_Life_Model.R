...
temp_dist <- function(n) {
  ans <- 6 #rlaplace(n, temp_location, temp_shape)
  #for(i in 1:n)
...
packages_sim$tot_strains <- 30 #pop_dist(n_sim)
...
packages_sim_exp <- packages_sim %>%
  ...
  mutate(isolate = NA, isolate_num = NA, isolate_rec = NA, 
         initial_count = NA, lag = NA, mumax = NA, nmax = NA) 

#Assigning isolates to each package, with replacement (i.e., 5 copies of each of the 6 rifR mutants in each package)
for(i in 1:n_packages){
  index <- which(packages_sim_exp$package == i)
  strain_per_package <- 6
  packages_sim_exp$isolate[index] <- rep(st_dist(strain_per_package), each = 5)
  packages_sim_exp$isolate_num[index] <- rep(1:5, times = 6)
...
#Creating a unique code to distiguish between isolates in a package (i.e., )
packages_sim_exp$isolate_rec <- paste0(packages_sim_exp$isolate, "_", 
                                        packages_sim_exp$isolate_num)
...
write.csv(count_summary_by_day, file = "sensitivity_analysis/trouble_shooting/outputs/inc.strains.temp_count_by_day.csv", row.names = FALSE)
write.csv(percent_packages_nmax, file = "sensitivity_analysis/trouble_shooting/outputs/inc.strains.temp_percentage_of_packages_at_nmax.csv", row.names = FALSE)
write.csv(counts_summary_by_package, file = "sensitivity_analysis/trouble_shooting/outputs/inc.strains.temp_total_count_by_package.csv", row.names = FALSE)
