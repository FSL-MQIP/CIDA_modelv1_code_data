...
NewLag <- function(temp, lag) {
  a <- lag*((6 - lag_tmin)/(temp - lag_tmin))^2
  ans <- a*1.6
  return(ans)
}
...
date <- Sys.Date()
date <- gsub("-", "_", date)
file_name_1 <- paste0("sensitivity_analysis/lag/output/lag_threshold_", date,".csv")
file_name_2 <- paste0("sensitivity_analysis/lag/output/lag_count_", date,".csv")
