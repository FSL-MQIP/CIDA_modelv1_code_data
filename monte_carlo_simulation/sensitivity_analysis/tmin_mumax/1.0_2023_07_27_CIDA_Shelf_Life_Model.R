...
for(i in 1:nrow(mumax_secon)){
  if(mumax_secon$isolate[i] != "s12-0116"){
    mumax_secon$mod_tmin[i] <- mumax_secon$temp_min_mu[i]*1.0
  } else {
    mumax_secon$mod_tmin[i] <- mumax_secon$temp_min_mu[i] - ((1-1.0)*100)*(abs(mumax_secon$temp_min_mu[i])/100)
  }
}
...
for(i in 1:nrow(packages_sim_exp)){
  ...
  strain_mu <- NewMu(packages_sim_exp$storage_temp[i], mumax_secon$b_mu[strain_index], mumax_secon$mod_tmin[strain_index], packages_sim_exp$isolate[i])
...
date <- Sys.Date()
date <- gsub("-", "_", date)
file_name_1 <- paste0("sensitivity_analysis/tmin_mumax/output/sa_mumax_tmin_threshold_", date,".csv")
file_name_2 <- paste0("sensitivity_analysis/tmin_mumax/output/sa_mumax_tmin_count_", date,".csv")
