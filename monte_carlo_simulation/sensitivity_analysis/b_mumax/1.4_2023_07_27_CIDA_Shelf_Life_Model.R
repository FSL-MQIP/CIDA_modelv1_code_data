...
for(i in 1:nrow(mumax_secon)){
  if(mumax_secon$isolate[i] != "s12-0141"){
    mumax_secon$mod_b[i] <- mumax_secon$b_mu[i]*1.4
  } else {
    mumax_secon$mod_b[i] <- mumax_secon$b_mu[i] - ((1-1.4)*100)*(abs(mumax_secon$b_mu[i])/100)
  }
}
...
for(i in 1:nrow(packages_sim_exp)){
  ...
  strain_mu <- NewMu(packages_sim_exp$storage_temp[i], mumax_secon$mod_b[strain_index], mumax_secon$temp_min_mu[strain_index], packages_sim_exp$isolate[i])
...
date <- Sys.Date()
date <- gsub("-", "_", date)
file_name_1 <- paste0("sensitivity_analysis/b_mumax/output/sa_b_mumax_threshold_", date,".csv")
file_name_2 <- paste0("sensitivity_analysis/b_mumax/output/sa_b_mumax_count_", date,".csv")
